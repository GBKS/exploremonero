<template>
  <div :class="classObject">
    <Transition name="receipt-details-transition" :duration="1000" appear>
      <div class="wrap">
        <div class="content" v-if="matchingOutput && transaction">
          <ol class="--two-columns">
            <li>
              <h5>{{ $t('general.amount_sent') }}</h5>
              <p>{{ amount }}</p>
            </li>
            <li>
          ...
          <ol>
            <li>
              <h5>{{ $t('general.recipient_address') }}</h5>
              <p>{{ address }}</p>
            </li>
            <li>
          ...
          <div class="options">
            <p><a href="#" class="button --medium" @click.prevent="clear">{{ $t('receipt.result.check_another_transaction') }}</a></p>
          </div>
        </div>
        <div class="content" v-if="!matchingOutput && transaction">
...
.receipt-details {
	...
	&.receipt-details-transition-enter-active,
	&.receipt-details-transition-leave-active {
		> .wrap {
			> .content {
				> ol {
					> li {
	...
	&.receipt-details-transition-enter-from {
		> .wrap {
			> .content {
				> ol {
					> li {
	...
	&.receipt-details-transition-enter-to {
		> .wrap {
			> .content {
				> ol {
					> li {
